...
public class Perform {
    private static Duration defaultElementIdentificationTimeout = Duration.ofSeconds(2);
    private static int attemptsBeforeThrowingElementNotFoundException = 2;
    private static int attemptsBeforeThrowingElementNotFoundException2 = 1;
   ...
   // static WebDriver lastUsedDriver = null;
	...
	try {
	    ...
	    passAction(driver, elementLocator, "clipboardActions", action);
	    		...
	    		Reporter.Log(e.toString());
	    Reporter.Log("Headless Exception: " + e.getMessage());
	}
    }
	...
	try {
	    typeAppend(driver, elementLocator,
		    (String) ((Toolkit.getDefaultToolkit().getSystemClipboard()).getContents(Perform.class))
			    .getTransferData(DataFlavor.stringFlavor));
	} catch (UnsupportedFlavorException e) {
	    		Reporter.Log(e.toString());
	    Reporter.Log("Unsupported Flavor Exception: " + e.getMessage());
	} catch (IOException e) {
	    		Reporter.Log(e.toString());
	    Reporter.Log("IO Exception: " + e.getMessage());
	}
    }
...
//    }
	...
	if (identifyUniqueElement(driver, elementLocator)) {
	    try {
		(new WebDriverWait(driver, defaultElementIdentificationTimeout .multipliedBy(numberOfTries)))
		...
		failAction(driver, "waitForTextToChange",
			"waited for (" + defaultElementIdentificationTimeout.multipliedBy(numberOfTries) + ") seconds");
	    }
	    try {
	...
	if (foundElementsCount <= 1) {
		...
		if (stateOfPresence) {
		    passAction(driver, elementLocator, "waitForElementToBePresent",
			    "waited for (" + defaultElementIdentificationTimeout .multipliedBy(numberOfTries)
	...
	public static boolean isElementExist(WebDriver driver, By elementLocator) {
		...
		for (int i = 0; i < attemptsBeforeThrowingElementNotFoundException2; i++) {
			try {
			    JSWaiter.waitForLazyLoading();
				...
				if (foundElementsCount > 0) {
					...
					| NoSuchElementException | TimeoutException e) {
				if (i + 1 == attemptsBeforeThrowingElementNotFoundException2) {
					Reporter.Log(e.toString());
					return false;
				}
			} catch (Exception e) {
				if ((e.getMessage().contains("cannot focus element"))
						&& (i + 1 == attemptsBeforeThrowingElementNotFoundException2)) {
					Reporter.Log(e.toString());
					return false;
				}
				Reporter.Log(e.toString());
